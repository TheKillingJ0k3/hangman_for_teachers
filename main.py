...
def open_excel(excel_file_path):  # creates excel the first time the game runs
    try:
        # , data_only=True in case file has a lot of formulas
...
def show_words():
    # , data_only=True in case file has a lot of formulas
    os.startfile(r".\\Hangman Excel\\Hangman Excel.xlsx")
...
def start_game(level):
    ...
    while word_selected == None:
        word_selected = ws.cell(row=(randint(2, ws.max_row)), column=int(
            levels_to_columns[level])).value
    # print(word_selected)
    hidden_word = word_selected.split()[1]
    hidden_word = word_selected.split()[0] + ' ' + word_selected.split(
    )[1][0] + ' ' + (len(hidden_word)-2)*'_ ' + word_selected.split()[1][-1]
    print(hidden_word)
    ...
    if word_selected.split()[1][-2] in german_alphabet:
        ...
        for letter in greek_alphabet:
            if greek_alphabet.index(letter) < 6:
                ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
                    letter)).grid(row=1, column=greek_alphabet.index(letter), sticky=W)
            elif greek_alphabet.index(letter) >= 6 and greek_alphabet.index(letter) < 12:
                ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
                    letter)).grid(row=2, column=(greek_alphabet.index(letter)-6), sticky=W)
            elif greek_alphabet.index(letter) >= 12 and greek_alphabet.index(letter) < 18:
                ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
                    letter)).grid(row=3, column=(greek_alphabet.index(letter)-12), sticky=W)
            elif greek_alphabet.index(letter) >= 18 and greek_alphabet.index(letter) < 24:
                ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
                    letter)).grid(row=4, column=(greek_alphabet.index(letter)-18), sticky=W)
            elif greek_alphabet.index(letter) >= 24:
                ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
                    letter)).grid(row=5, column=(greek_alphabet.index(letter)-24), sticky=W)
    Label(attempt_frame, textvariable=tkinter_attempts).pack(
        side=LEFT, anchor=SE)
...
def select_letter(letter):  # play round
    ...
    if tkinter_letter_selected in word_selected:
        ...
        # print(word_selected)
        # print(hidden_word)
        # 1, -1 because we don't care about first and last letter
        ...
        # disable button
        ttk.Button(letter_frame, text=str(letter), width=2, command=lambda letter=letter: select_letter(
            letter))['state'] = DISABLED  # not working
    else:
        print("Try again!")
        attempts = attempts - 1
        print(attempts)
    ...
    if ''.join(word_selected.split()) == ''.join(hidden_word.split()) and attempts > 0:
        ...
        # .geometry(f"dimension+{root.winfo_x()}+{root.winfo_y()}")
        messagebox.showinfo(title='Dude, stop!',
                            message='How about playing a new game instead?')
...
# same as frame['menu'] = menubar, doesn't need menu=menu_file etc inside cascade
...
# creates name of new submenu
menubar.add_cascade(label='New Game', menu=action_menu)
level_menu = Menu(action_menu, tearoff=False)
# adds option to submenu
level_menu.add_command(label='A1', command=start_level_A1)
# adds option to submenu
level_menu.add_command(label='A2', command=start_level_A2)
# adds option to submenu
level_menu.add_command(label='B1', command=start_level_B1)
# adds option to submenu
level_menu.add_command(label='B2', command=start_level_B2)
# adds option to submenu
level_menu.add_command(label='C1', command=start_level_C1)
# adds option to submenu
level_menu.add_command(label='C2', command=start_level_C2)
# creates name of new submenu
action_menu.add_cascade(label='Choose Level', menu=level_menu)
...
# creates name of new submenu
settings_menu.add_command(label='Add Words', command=show_words)
...
# creates name of new submenu
menubar.add_cascade(label='Advice', menu=advice_menu)
